...
P_ref0=np.zeros((nat,nat))
f=open(reffile[-1],'r')
lsf=f.readlines()
for j in range(len(lsf)):
    ls_f=lsf[j].strip('\n').split()
    P_ref0[int(ls_f[0])-1,int(ls_f[1])-1]=float(ls_f[2])
CS_ref0=np.heaviside(cpmt_sgnl(P_ref0)[0],0.5)
...
CS_ref=np.zeros(np.shape(P_ref)[:-1])
for i in range(len(reffile)):
    CS_ref[i]=np.heaviside(cpmt_sgnl(P_ref[i])[0],0.5)
    if np.corrcoef(CS_ref[i],CS_ref0)[0,1]<0:
        CS_ref[i]*=-1

P=np.load(npyfile)
CS=np.zeros(np.shape(P)[:-1])
for i in range(nsw):
    for j in range(nfr+2):
        CS[i,j]=np.heaviside(cpmt_sgnl(P[i,j])[0],0.5)
        if np.corrcoef(CS[i,j],CS_ref0)[0,1]<0:
            CS[i,j]*=-1
CS=CS.reshape(nsw*(nfr+2),-1)

CS=np.concatenate((CS,CS_ref),axis=0)

pca=PCA(n_components=2)
pca.fit(CS)
pc_nor=pca.transform(CS)
