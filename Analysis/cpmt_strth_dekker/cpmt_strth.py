...
for i in range(nsw):
    for j in range(ncl):
        P_nor=cpmt_sgnl(P_ref[i,j])[1][sort_idx][:,sort_idx]
        P_nor[P_nor==0]=np.min(P_nor[P_nor>0])/2
        P_nor=np.log2(P_nor)
        # P_nor=np.ma.corrcoef(P_nor)
        CSs_ref[i,j,0]=np.mean(P_nor[-n2:,-n2:])
        CSs_ref[i,j,1]=np.mean(P_nor[:n1,:n1])
        CSs_ref[i,j,2]=np.mean(P_nor[-n2:,:n1])

P=np.load(npyfile)
CSs=np.zeros((nsw,nfr,3))
for i in range(nsw):
    for j in range(1,nfr+1):
        P_nor=cpmt_sgnl(P[i,j])[1][sort_idx][:,sort_idx]
        P_nor[P_nor==0]=np.min(P_nor[P_nor>0])/2
        P_nor=np.log2(P_nor)
        # P_nor=np.ma.corrcoef(P_nor)
        CSs[i,j-1,0]=np.mean(P_nor[-n2:,-n2:])
        CSs[i,j-1,1]=np.mean(P_nor[:n1,:n1])
        CSs[i,j-1,2]=np.mean(P_nor[-n2:,:n1])
