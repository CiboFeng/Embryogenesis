    ...
    for j in range(nat):
        for k in range(j):
            if abs(j-k) in range(scl[i][0],scl[i][1]):
                P_temp+=[P0[:,:,j,k]]
    Ps0+=[np.transpose(np.array(P_temp),(1,2,0))]
        ...
        for k in range(nat):
            for l in range(k):
                if abs(k-l) in range(scl[j][0],scl[j][1]):
                    P_temp+=[P[:,:,k,l]]
        Ps=np.transpose(np.array(P_temp),(1,2,0)).reshape(nfr*ntr[i],-1)
        Ps=np.concatenate((Ps0[j][i],Ps),axis=0)
        pca=PCA(n_components=1)
        pca.fit(Ps)
        pc=pca.transform(Ps)[:,0]
        # perct=pca.explained_variance_ratio_
        Q0[i,:,j]=pc[:ncl]
        Q[i][:,:,j]=pc[ncl:].reshape(nfr,ntr[i])
