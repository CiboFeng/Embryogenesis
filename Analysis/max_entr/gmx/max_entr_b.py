...
def PC(i,q1,q2,dir,files,dt,r0,hipair):
    """Calculate the contact probability matrix as a flat vector and the cross-averaged matrix."""

    """Read All .trj Files in Directory and Integrate Them into a Trajectory"""
    x=[]
    y=[]
    z=[]
    x+=pdbs(dir+f'/{files[i]}')[1]
    y+=pdbs(dir+f'/{files[i]}')[2]
    z+=pdbs(dir+f'/{files[i]}')[3]

    """Calculate and Deliver Data"""
    dirls=dir.split('/')
    wkdir=''
    for i in range(len(dirls)-1):
        wkdir+=dirls[i]
        wkdir+='/'
    P,C=cont(np.array(x)/10,np.array(y)/10,np.array(z)/10,d0=r0,reflect='cos/sqr',hipair=hipair,beta=3,wkdir=wkdir).intg_m()
    q1.put(P)
    q2.put(C)
...
def max_entr_b(sysname,imapfile,dir,checkfile,matfile,omapfile,pmin,dt,r0,tol):
    ...
    with multiprocessing.Manager() as manager:
        ...
        for i in range(len(files)):
            p=multiprocessing.Process(target=PC,args=(i,q1,q2,dir,files,dt,r0,hipair))
            p.start()
            procs.append(p)
        for p in procs:
            p.join()
