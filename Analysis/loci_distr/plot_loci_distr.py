...
for i in range(nsw):
    for j in range(len(idx1)):
        ...
        for k in range(ncl):
            ...
            ax[i,j].plot(r0[i,k,:,idx1[j]],rho0[i,k,:,idx1[j]],color=color0[k][i],linestyle='-',linewidth=wth,label=label0[k][i])
...
for i in range(nsw):
    for j in range(len(idx2)):
        ...
        for k in range(ncl):
            ...
            ax[i,j].plot(r0[i,k,:,idx2[j]],rho0[i,k,:,idx2[j]],color=color0[k][i],linestyle='-',linewidth=wth,label=label0[k][i])
        ax[i,j].minorticks_on()
        ax[i,j].tick_params(axis='both',which='major',direction='in',width=wth,length=lenmaj,labelsize=size)
        ax[i,j].tick_params(axis='both',which='minor',direction='in',width=wth,length=lenmin,labelsize=size)
        # ax[i,j].xaxis.set_major_locator(MultipleLocator(xtick))
...
for i in range(nsw):
    ...
    for j in range(len(idx1)):
        ...
        for k in range(ncl):
            ...
            if k==0:
                ax[i,0].plot([min(t[1:-9]),t0-gap0],[r0_mean[i,k,idx1[j]],r0_mean[i,k,idx1[j]]],color=color0[k][i],linestyle='--',linewidth=wth)
            if k==1:
                ax[i,0].plot([t0+gap0,max(t[1:-9])],[r0_mean[i,k,idx1[j]],r0_mean[i,k,idx1[j]]],color=color0[k][i],linestyle='--',linewidth=wth)
    for k in range(ncl):
        ax[i,0].plot([],[],color=color0[k][i],linestyle='--',linewidth=wth,label=label0[k][i])
    for j in range(len(idx1)):
...
for i in range(nsw):
    ...
    for j in range(len(idx1)):
        ...
        for k in range(ncl):
            ...
            gap0=np.exp(0.1*(np.log(min(t[1:-9]))-np.log(max(t[1:-9]))))
            if k==0:
                ax[i+2,0].plot([min(t[1:-9]),t0-gap0],[r0_std[i,k,idx1[j]],r0_std[i,k,idx1[j]]],color=color0[k][i],linestyle='--',linewidth=wth)
            if k==1:
                ax[i+2,0].plot([t0+gap0,max(t[1:-9])],[r0_std[i,k,idx1[j]],r0_std[i,k,idx1[j]]],color=color0[k][i],linestyle='--',linewidth=wth)
    for k in range(ncl):
        ax[i+2,0].plot([],[],color=color0[k][i],linestyle='--',linewidth=wth,label=label0[k][i])
    for j in range(len(idx1)):
...
for i in range(nsw):
    ...
    for j in range(len(idx2)):
        ...
        for k in range(ncl):
            ...
            gap0=np.exp(0.1*(np.log(min(t[1:-9]))-np.log(max(t[1:-9]))))
            if k==0:
                ax[i,1].plot([min(t[1:-9]),t0-gap0],[r0_mean[i,k,idx2[j]],r0_mean[i,k,idx2[j]]],color=color0[k][i],linestyle='--',linewidth=wth)
            if k==1:
                ax[i,1].plot([t0+gap0,max(t[1:-9])],[r0_mean[i,k,idx2[j]],r0_mean[i,k,idx2[j]]],color=color0[k][i],linestyle='--',linewidth=wth)
    for k in range(ncl):
        ax[i,1].plot([],[],color=color0[k][i],linestyle='--',linewidth=wth,label=label0[k][i])
    for j in range(len(idx2)):
...
for i in range(nsw):
    ...
    for j in range(len(idx2)):
        ...
        for k in range(ncl):
            ...
            gap0=np.exp(0.1*(np.log(min(t[1:-9]))-np.log(max(t[1:-9]))))
            if k==0:
                ax[i+2,1].plot([min(t[1:-9]),t0-gap0],[r0_std[i,k,idx2[j]],r0_std[i,k,idx2[j]]],color=color0[k][i],linestyle='--',linewidth=wth)
            if k==1:
                ax[i+2,1].plot([t0+gap0,max(t[1:-9])],[r0_std[i,k,idx2[j]],r0_std[i,k,idx2[j]]],color=color0[k][i],linestyle='--',linewidth=wth)
    for k in range(ncl):
        ax[i+2,1].plot([],[],color=color0[k][i],linestyle='--',linewidth=wth,label=label0[k][i])
    for j in range(len(idx2)):
