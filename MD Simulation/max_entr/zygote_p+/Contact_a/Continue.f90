...
parameter(cutoff1=0.005,cutoff2=0.005)
real cutoff_GD
...
do i=1,Nres,3
        do j=1,Nres,3
!                if(abs(j-i)<=cutoff_GD/unit) then
                 if(abs(j-i)>5/unit) then
                        if(ContactPCG(i,j)==0) then
!                        if(ContactP(i,j)>=0.0001) then
!                        if((ContactP(i,j)-Prob(i,j))/(ContactP(i,j))>0.3.or.(ContactP(i,j)-Prob(i,j))/(ContactP(i,j))<-0.3) then
                        if(ContactP(i,j)<Prob(i,j)) then
                                Contact_a(i,j)=-0.011
                                ContactPCG(i,j)=ContactP(i,j)
!                        endif
                                endif
